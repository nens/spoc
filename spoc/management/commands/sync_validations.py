    ...
    def handle(self, *args, **options):
        ...
        with open(filepath, 'rb') as f:
            ...
            for row in reader:
                ...
                self.bind_formula_to_parameter(row)
        self.stdout.write('Successfully passed.')

    def bind_formula_to_parameter(self, row):
        """Add a formula if it not exists."""
        if row['Formule'] == '':
            return
        parameter = models.Parameter.objects.get(id=row['WNS-code'])
        parameter.formula_allowed = True
        parameter.save()
        self.stdout.write("Parameter {} allowed to have formulas".format(parameter.id))
