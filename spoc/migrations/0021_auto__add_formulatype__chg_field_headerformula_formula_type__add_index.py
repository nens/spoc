...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'FormulaType'
        db.create_table(u'spoc_formulatype', (
            ('code', self.gf('django.db.models.fields.CharField')(max_length=20, primary_key=True)),
        ))
        db.send_create_signal(u'spoc', ['FormulaType'])


        # Renaming column for 'HeaderFormula.formula_type' to match new field type.
        db.rename_column(u'spoc_headerformula', 'formula_type', 'formula_type_id')
        # Changing field 'HeaderFormula.formula_type'
        db.alter_column(u'spoc_headerformula', 'formula_type_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['spoc.FormulaType']))
        # Adding index on 'HeaderFormula', fields ['formula_type']
        db.create_index(u'spoc_headerformula', ['formula_type_id'])


    def backwards(self, orm):
        # Removing index on 'HeaderFormula', fields ['formula_type']
        db.delete_index(u'spoc_headerformula', ['formula_type_id'])

        # Deleting model 'FormulaType'
        db.delete_table(u'spoc_formulatype')


        # Renaming column for 'HeaderFormula.formula_type' to match new field type.
        db.rename_column(u'spoc_headerformula', 'formula_type_id', 'formula_type')
        # Changing field 'HeaderFormula.formula_type'
